...
function countProfit(shoppers) {
  ...
  // buat object dagangan
  if (shoppers.length !== 0){
    var objResult=[]
    for (var h = 0; h < listBarang.length; h++) {
      var objectBarang={}
        objectBarang.product = listBarang[h][0];
        objectBarang.shoppers = [];
        for (var i = 0; i < shoppers.length; i++) {
          if (shoppers[i].product ===  listBarang[h][0] && shoppers[i].amount <= listBarang[h][2]) {
            objectBarang.shoppers.push(shoppers[i].name);
            objectBarang.leftOver = listBarang[h][2] - shoppers[i].amount;
            objectBarang.totalProfit = shoppers[i].amount * listBarang[h][1];
          }
          else {
            objectBarang.leftOver = listBarang[h][1];
            objectBarang.totalProfit = 0;
          }
      }
        objResult.push(objectBarang);
        objectBarang = {};
    }
    return objResult;
  } else{
